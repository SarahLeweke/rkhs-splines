function plotExportPGFSphere(xData, yData, zData, cData, fileName, viewAngles, axisLimits, labels, hasColorbar, tikzSubPath)
...
%                fprintf(fid, '\tcolormap={changepoint}{[1pt]\nrgb255(%dpt)=(28,23,69)\nrgb255(-1pt)=(25,25,122)\n', lowerPoint);
    ...
    fprintf(fid, ']\n');
    fprintf(fid, '\\addplot3[surf,shader=interp,z buffer=sort,row sep=crcr,mesh/ordering=rowwise,mesh/cols=%d,%s] table[x expr={100*\\thisrowno{0}},y expr={100*\\thisrowno{1}},z expr={100*\\thisrowno{2}},%sheader=false] {%s%s.dat};\n', size(xData, 2), pointMetaSrc, metaIndex, tikzSubPath, baseName);
    fprintf(fid, '\n\\end{axis}\n\\end{tikzpicture}%%');
    fclose(fid);
end
